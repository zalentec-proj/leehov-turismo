...
- A troca e a renovação usam o endpoint oficial OAuth `https://api.rd.services/oauth2/token` com formulário URL-encoded.
- O parser do webhook agrega `contacts`, `contact_ids`, `contact_id` e `person`. Essa decisão cobre o payload CRM v2 que pode trazer a relação `contacts` vazia mesmo com `contact_ids` preenchido, sem registrar o corpo recebido. Se o webhook não trouxer nenhum vínculo de contato, a aplicação consulta a negociação atual pela API CRM OAuth e aproveita somente seus IDs de contato; se a consulta falhar, a venda continua ignorada.
- O valor é lido de `amount_total` no webhook e também de `total_price` na leitura direta da negociação CRM v2. Isso mantém a regra de valor positivo idêntica tanto no recebimento quanto no reenvio/fallback.
- O botão administrativo **Configurar webhook do RD** cria, após confirmação do administrador, apenas `crm_deal_updated` em `https://leehovturismo.com.br/api/integrations/rd/meta-purchase`, método `POST`, usando o cabeçalho `x-leehov-rd-webhook-key`. A ação usa a API oficial **RD Station CRM v2** (`/crm/v2/webhooks`) e o token OAuth do aplicativo CRM; ela consulta as assinaturas antes de criar, evitando duplicidade.
- Se a criação ou consulta da assinatura falhar, o servidor registra somente uma categoria fixa e, quando houver, o status HTTP do RD. Tokens, URLs, corpo de resposta e payloads não são registrados. O painel mostra o mesmo código seguro para orientar o diagnóstico.
- Em caso de falha OAuth, o servidor registra apenas uma categoria técnica segura para diagnóstico; códigos de autorização, tokens e payloads não são registrados.
