...
- O parser do webhook agrega `contacts`, `contact_ids`, `contact_id` e `person`. Essa decisão cobre o payload CRM v2 que pode trazer a relação `contacts` vazia mesmo com `contact_ids` preenchido, sem registrar o corpo recebido.
