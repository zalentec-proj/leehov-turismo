              ...
              {related.map((item) => (
                <Link key={item.id} href={`/blog/${item.slug}`} className="group overflow-hidden rounded-[18px] border border-leehov-border bg-white shadow-leehov-card">
                  <div className="relative aspect-[16/10] overflow-hidden bg-leehov-sky">
                    {item.imageUrl ? <Image src={item.imageUrl} alt={item.coverAltText} fill quality={86} sizes="(min-width: 768px) 33vw, 100vw" className="object-cover transition duration-500 group-hover:scale-[1.035] motion-reduce:transition-none" /> : null}
                  </div>
                  <div className="p-6">
