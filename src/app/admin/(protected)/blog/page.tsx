...
export default async function AdminBlogPage() {
  const { permissions } = await requirePermission("blog.view");
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        ...
        {permissions.includes("blog.create") ? <Button asChild size="lg" className="self-start rounded-full bg-leehov-blue-600 px-6 text-white hover:bg-leehov-cyan lg:self-auto"><Link href="/admin/blog/novo"><Plus className="size-4" />Novo post</Link></Button> : null}
      </header>
      ...
      <AdminBlogTable data={posts} categories={categories} canCreate={permissions.includes("blog.create")} canUpdate={permissions.includes("blog.update")} canPublish={permissions.includes("blog.publish")} canDeleteDraft={permissions.includes("blog.delete_draft")} />
      {permissions.includes("blog.manage_categories") ? <BlogCategoryManager categories={categories} /> : null}
    </div>
  );
}
