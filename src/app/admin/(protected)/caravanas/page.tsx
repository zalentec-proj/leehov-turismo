...
export default async function AdminCaravansPage() {
  ...
  return <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-leehov-blue-600">Conteúdo</p><h2 className="mt-3 text-3xl font-extrabold text-leehov-navy-950">Pacotes</h2><p className="mb-8 mt-3 text-sm leading-7 text-leehov-muted">Gerencie roteiros, saídas, imagens, publicação e destaques. Rascunhos podem ser excluídos definitivamente após confirmação.</p>{access.permissions.includes("caravans.manage_categories") ? <CategoryManager categories={categories} /> : null}<AdminCaravansTable data={caravans} canCreate={access.permissions.includes("caravans.create")} canUpdate={access.permissions.includes("caravans.update")} canPublish={access.permissions.includes("caravans.publish")} /></div>;
}
