...
export default async function AdminLeadsPage() {
  ...
  return <><div className="mb-8"><p className="text-xs font-bold uppercase tracking-[0.18em] text-leehov-blue-600">Relacionamento</p><h2 className="mt-3 text-3xl font-extrabold text-leehov-navy-950">Leads</h2><p className="mt-3 text-leehov-muted">Acompanhe contatos e interesses em pacotes sem excluir o histórico.</p></div><div className="mb-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{cards.map((item) => <Card key={item.label} className="rounded-[18px] border-leehov-border p-5 shadow-leehov-card"><item.icon className="size-6 text-leehov-blue-500" /><p className="mt-5 text-3xl font-extrabold text-leehov-navy-950">{item.value}</p><p className="mt-1 text-sm text-leehov-muted">{item.label}</p></Card>)}</div><AdminLeadsTable data={leads} owners={owners} caravans={caravans.map((item) => ({ id: item.id, title: item.title }))} whatsappTemplate={settings.whatsapp.caravanTemplate} consultantName={access.profile.name || "equipe Leehov"} /></>;
}
