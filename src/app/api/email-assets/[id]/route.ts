...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: asset } = await admin.from("media_assets").select("storage_path").eq("id", id).maybeSingle();
  if (!asset) return new NextResponse("Imagem não encontrada.", { status: 404 });
  const { data, error } = await admin.storage.from("site-media").createSignedUrl(asset.storage_path, 300);
