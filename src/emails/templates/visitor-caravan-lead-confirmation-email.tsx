  ...
  return (
    <EmailLayout preview={`Recebemos seu interesse em ${props.caravanTitle}`}>
      ...
      <p style={{ fontSize: "16px", lineHeight: "26px", margin: 0 }}>
        Olá, {props.name}. A equipe Leehov recebeu seu interesse no pacote <strong>{props.caravanTitle}</strong> e entrará em contato para orientar os próximos passos.
      </p>
      <EmailButton href={props.whatsappUrl || props.caravanUrl}>{props.whatsappUrl ? "Conversar no WhatsApp" : "Rever o pacote"}</EmailButton>
    </EmailLayout>
  );
}
