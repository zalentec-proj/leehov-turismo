...
export function AdminCaravansTable({ data, canCreate, canUpdate, canPublish }: { data: CaravanSummary[]; canCreate: boolean; canUpdate: boolean; canPublish: boolean }) {
  ...
  const columns = useMemo(() => [
    columnHelper.accessor("title", { header: "Pacote", cell: ({ row }) => <div className="flex min-w-[260px] items-center gap-3"><div className="relative size-14 shrink-0 overflow-hidden rounded-xl bg-leehov-surface">{row.original.imageUrl ? <Image src={row.original.imageUrl} alt="" fill sizes="56px" quality={78} className="object-cover" /> : null}</div><div><p className="font-bold text-leehov-navy-950">{row.original.title}</p><p className="text-xs text-leehov-muted">{row.original.destination}</p></div></div> }),
    ...
    columnHelper.display({ id: "publication", header: "Publicação", cell: ({ row }) => canPublish ? <Button type="button" variant="outline" size="sm" disabled={pending} onClick={() => startTransition(async () => { const result = await setCaravanPublishedAction(row.original.id, !row.original.published); if (result.success) toast.success(result.message); else toast.error(result.message); router.refresh(); })}>{pending ? <Loader2 className="size-3 animate-spin" /> : null}{row.original.published ? "Despublicar" : "Publicar"}</Button> : <span className="text-xs text-leehov-muted">Somente leitura</span> }),
    columnHelper.display({ id: "actions", header: "", cell: ({ row }) => canUpdate ? <div className="flex items-center justify-end gap-1"><Button asChild variant="ghost" size="icon"><Link href={`/admin/caravanas/${row.original.id}`} aria-label={`Editar ${row.original.title}`}><Edit3 className="size-4" /></Link></Button>{!row.original.published && row.original.status === "draft" ? <Button type="button" variant="ghost" size="icon" disabled={pending} aria-label={`Excluir ${row.original.title}`} onClick={() => { if (!window.confirm(`Excluir definitivamente o rascunho “${row.original.title}” e todas as suas imagens?`)) return; startTransition(async () => { const result = await deleteDraftCaravanAction(row.original.id); if (result.success) toast.success(result.message); else toast.error(result.message); router.refresh(); }); }}><Trash2 className="size-4 text-destructive" /></Button> : null}</div> : null }),
  ], [canPublish, canUpdate, pending, router]);
  ...
  return <div className="space-y-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex flex-1 gap-3"><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por pacote, destino ou categoria" className="max-w-lg" /><Select value={status} onValueChange={setStatus}><SelectTrigger className="w-[180px]"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Todos os status</SelectItem><SelectItem value="draft">Rascunho</SelectItem><SelectItem value="available">Disponível</SelectItem><SelectItem value="coming_soon">Em breve</SelectItem><SelectItem value="waitlist">Lista de espera</SelectItem><SelectItem value="sold_out">Esgotada</SelectItem></SelectContent></Select></div>{canCreate ? <Button asChild className="rounded-full bg-leehov-blue-600 text-white"><Link href="/admin/caravanas/novo"><Plus className="size-4" />Novo pacote</Link></Button> : null}</div><Card className="overflow-hidden rounded-[18px] border-leehov-border p-0"><div className="overflow-x-auto"><Table><TableHeader>{table.getHeaderGroups().map((group) => <TableRow key={group.id}>{group.headers.map((header) => <TableHead key={header.id}>{header.isPlaceholder ? null : flexRender(header.column.columnDef.header, header.getContext())}</TableHead>)}</TableRow>)}</TableHeader><TableBody>{table.getRowModel().rows.map((row) => <TableRow key={row.id}>{row.getVisibleCells().map((cell) => <TableCell key={cell.id}>{flexRender(cell.column.columnDef.cell, cell.getContext())}</TableCell>)}</TableRow>)}{!table.getRowModel().rows.length ? <TableRow><TableCell colSpan={columns.length} className="h-32 text-center text-leehov-muted">Nenhum pacote encontrado.</TableCell></TableRow> : null}</TableBody></Table></div></Card></div>;
}
