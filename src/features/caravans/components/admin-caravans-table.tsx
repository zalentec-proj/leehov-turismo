...
export function AdminCaravansTable({ data, canCreate, canUpdate, canPublish }: { data: AdminCaravan[]; canCreate: boolean; canUpdate: boolean; canPublish: boolean }) {
  ...
  const columns = useMemo(() => [
    ...
    columnHelper.display({ id: "actions", header: "", cell: ({ row }) => canUpdate ? <Button asChild variant="ghost" size="icon"><Link href={`/admin/caravanas/${row.original.id}`} aria-label={`Editar ${row.original.title}`}><Edit3 className="size-4" /></Link></Button> : null }),
