...
const sourceLabels: Record<LeadSource, string> = { contact: "Contato", caravan_interest: "Interesse em caravana", popup: "Pop-up", manual: "Manual", whatsapp: "WhatsApp", phone: "Telefone", referral: "Indicação", social: "Rede social", other: "Outro" };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-3 lg:grid-cols-3 xl:grid-cols-6">
        ...
        <Filter value={caravan} onChange={setCaravan} all="Todas as caravanas" items={caravans.map((item) => [item.id, item.title])} />
...
function ManualLeadDialog({ open, onOpenChange, owners, caravans }: { open: boolean; onOpenChange: (open: boolean) => void; owners: LeadOwner[]; caravans: Array<{ id: string; title: string }> }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>Novo lead</DialogTitle><DialogDescription>Cadastre um contato recebido fora dos formulários do site.</DialogDescription></DialogHeader><div className="grid gap-4 sm:grid-cols-2"><Field label="Nome *"><Input value={form.name} onChange={(event) => set("name", event.target.value)} /></Field><Field label="WhatsApp *"><Input value={form.phone} onChange={(event) => set("phone", event.target.value)} /></Field><Field label="E-mail"><Input type="email" value={form.email} onChange={(event) => set("email", event.target.value)} /></Field><Field label="Origem"><Filter value={form.source} onChange={(value) => set("source", value)} all="Origem" showAll={false} items={manualSources.map((item) => [item, sourceLabels[item]])} /></Field><Field label="Cidade"><Input value={form.city} onChange={(event) => set("city", event.target.value)} /></Field><Field label="Estado"><Input value={form.state} onChange={(event) => set("state", event.target.value)} /></Field><Field label="Caravana"><Filter value={form.caravanId || "none"} onChange={(value) => set("caravanId", value === "none" ? "" : value)} all="Selecione" showAll={false} items={[["none", "Sem caravana"], ...caravans.map((item) => [item.id, item.title])]} /></Field><Field label="Responsável"><Filter value={form.assignedTo || "none"} onChange={(value) => set("assignedTo", value === "none" ? "" : value)} all="Selecione" showAll={false} items={[["none", "Não atribuído"], ...owners.map((item) => [item.id, item.name])]} /></Field><Field label="Próximo contato"><Input type="datetime-local" value={form.nextFollowUpAt} onChange={(event) => set("nextFollowUpAt", event.target.value)} /></Field><div className="sm:col-span-2"><Field label="Mensagem"><Textarea rows={4} value={form.message} onChange={(event) => set("message", event.target.value)} /></Field></div></div><Button disabled={pending} onClick={save}>{pending ? "Salvando…" : "Cadastrar lead"}</Button></DialogContent></Dialog>;
}
