...
export function AdminMediaLibrary({ assets }: { assets: MediaAsset[] }) {
  ...
  return <div className="space-y-6"><Card className="rounded-[18px] border-leehov-border p-6 shadow-leehov-card"><form ref={formRef} action={upload} className="grid gap-4 lg:grid-cols-[1.2fr_180px_1fr_1fr_auto] lg:items-end"><div><Label htmlFor="media-file">Imagem</Label><Input id="media-file" name="file" type="file" accept="image/jpeg,image/png,image/webp,image/avif" required className="mt-2" /></div><div><Label>Pasta</Label><Select name="folder" defaultValue="general"><SelectTrigger className="mt-2 w-full"><SelectValue /></SelectTrigger><SelectContent>{mediaFolders.map((item) => <SelectItem key={item} value={item}>{folderLabels[item]}</SelectItem>)}</SelectContent></Select></div><div><Label htmlFor="media-alt">Texto alternativo</Label><Input id="media-alt" name="altText" className="mt-2" placeholder="Descreva a imagem" /></div><div><Label htmlFor="media-caption">Legenda</Label><Input id="media-caption" name="caption" className="mt-2" placeholder="Legenda opcional" /></div><Button disabled={pending} className="h-9 bg-leehov-blue-600"><Upload />{pending ? "Enviando…" : "Enviar"}</Button></form><p className="mt-3 text-xs text-leehov-muted">JPEG, PNG, WebP ou AVIF, até 8 MiB. SVG e arquivos com assinatura inválida são recusados.</p></Card><div className="grid gap-3 md:grid-cols-[1fr_220px]"><div className="relative"><Search className="absolute left-3 top-2.5 size-4 text-leehov-muted" /><Input value={query} onChange={(event) => { setQuery(event.target.value); setVisibleCount(PREVIEW_BATCH_SIZE); }} className="pl-9" placeholder="Buscar por nome, alt ou legenda" /></div><Select value={folder} onValueChange={(value) => { setFolder(value); setVisibleCount(PREVIEW_BATCH_SIZE); }}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Todas as pastas</SelectItem>{mediaFolders.map((item) => <SelectItem key={item} value={item}>{folderLabels[item]}</SelectItem>)}</SelectContent></Select></div><div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">{visibleAssets.map((asset) => <MediaCard key={asset.id} asset={{ ...asset, signedUrl: previewUrls[asset.id] ?? asset.signedUrl }} />)}{!filtered.length ? <Card className="col-span-full rounded-[18px] border-leehov-border p-12 text-center text-leehov-muted">Nenhuma imagem encontrada.</Card> : null}</div>{visibleAssets.length < filtered.length ? <div className="flex justify-center"><Button type="button" variant="outline" onClick={() => setVisibleCount((count) => count + PREVIEW_BATCH_SIZE)}>Carregar mais imagens</Button></div> : null}</div>;
}
