    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-hidden sm:max-w-5xl">
        ...
        <div className="grid gap-3 sm:grid-cols-[1fr_220px]">
          <div className="relative"><Search className="absolute left-3 top-2.5 size-4 text-leehov-muted" /><Input value={query} onChange={(event) => { setQuery(event.target.value); setVisibleCount(24); }} className="pl-9" placeholder="Buscar por arquivo, origem ou tag" /></div>
          <Select value={folder} onValueChange={(value) => { setFolder(value); setVisibleCount(24); }}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Todas as pastas</SelectItem>{mediaFolders.map((item) => <SelectItem key={item} value={item}>{folderLabels[item]}</SelectItem>)}</SelectContent></Select>
        </div>
        <div className="grid max-h-[62vh] gap-4 overflow-y-auto pr-1 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {previewAssets.length < filtered.length ? <div className="col-span-full flex justify-center"><Button type="button" variant="outline" onClick={() => setVisibleCount((count) => count + 24)}>Carregar mais imagens</Button></div> : null}
        </div>
