...
const popupSelect = "*, image:media_assets(id, storage_bucket, storage_path, alt_text), caravan:caravans(id, title, slug, published)";

async function mapPopup(row: Record<string, unknown>): Promise<Popup> {
  const image = row.image as { id?: string; storage_bucket?: "site-media" | "caravan-images" | "blog-images"; storage_path?: string; alt_text?: string | null } | null;
