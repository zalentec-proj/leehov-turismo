...
const homeFallback = {
  ...
  testimonialsEyebrow: "Avaliado por quem viaja conosco",
...
const emailFallback: EmailSettings = {
  enabled: true,
  visitorConfirmationsEnabled: true,
  contactRecipients: [],
  leadRecipients: [],
  senderName: "Leehov Turismo",
  replyTo: "",
  footerText: "Leehov Turismo",
  whatsapp: "",
};
...
const listValue = (record: Record<string, unknown>, key: string) =>
...
export async function getPublicSiteSettings(): Promise<PublicSiteSettings> {
  ...
  const { data } = await supabase
    .from("site_settings")
    .select("key, value, media_asset_id")
    .eq("public_read", true);
  const rows = rowMap(
  ...
  const resolved = {
    ...
    whatsapp: {
      ...
      defaultMessage: textValue(
        whatsapp,
        "defaultMessage",
        whatsappFallback.defaultMessage,
      ),
      caravanMessage: textValue(
        whatsapp,
        "caravanMessage",
    ...
    social: {
      ...
      facebook: safeHttpsUrl(textValue(social, "facebook")),
      youtube: safeHttpsUrl(textValue(social, "youtube")),
    },
    home: {
      videoUrl:
        safeHttpsUrl(textValue(home, "videoUrl")) || homeFallback.videoUrl,
      testimonialsEyebrow: textValue(
        home,
        "testimonialsEyebrow",
        homeFallback.testimonialsEyebrow,
      ),
      testimonialsTitle: textValue(
        home,
        "testimonialsTitle",
        homeFallback.testimonialsTitle,
      ),
    },
    seo: {
      siteName: textValue(seo, "siteName", seoFallback.siteName),
      titleTemplate: textValue(seo, "titleTemplate", seoFallback.titleTemplate),
    ...
    consent: {
      ...
      version: numberValue(consent, "version", 1),
      durationDays: numberValue(consent, "durationDays", 180),
    },
  };
  return {
    ...resolved,
    whatsappNumber: resolved.whatsapp.number,
    whatsappDefaultMessage: resolved.whatsapp.defaultMessage,
    contactEmail: resolved.contact.contactEmail,
  };
}
...
export async function getAdminSiteSettings(): Promise<AdminSiteSettings> {
  ...
  const [publicSettings, privateRows] = await Promise.all([
    ...
    createAdminClient()
      .from("site_settings")
      .select("key, value")
      .in("key", ["email_settings", "analytics_settings"]),
  ]);
  const rows = rowMap(
    (privateRows.data ?? []) as Array<{ key: string; value: unknown }>,
  );
  ...
  return {
    ...publicSettings,
    email: {
      enabled: boolValue(email, "enabled", emailFallback.enabled),
      visitorConfirmationsEnabled: boolValue(
        email,
        "visitorConfirmationsEnabled",
        emailFallback.visitorConfirmationsEnabled,
      ),
      contactRecipients: listValue(email, "contactRecipients"),
      leadRecipients: listValue(email, "leadRecipients"),
      senderName: textValue(email, "senderName", emailFallback.senderName),
      replyTo: textValue(email, "replyTo"),
      footerText: textValue(email, "footerText", emailFallback.footerText),
      whatsapp: textValue(email, "whatsapp"),
    },
    tracking: {
      gaMeasurementId: validTrackingId(
        textValue(tracking, "gaMeasurementId"),
        /^G-[A-Z0-9]{4,20}$/,
      ),
      gtmContainerId: validTrackingId(
        textValue(tracking, "gtmContainerId"),
        /^GTM-[A-Z0-9]{4,20}$/,
      ),
      metaPixelId: validTrackingId(
        textValue(tracking, "metaPixelId"),
        /^[0-9]{5,30}$/,
      ),
    },
  };
...
export async function getServerEmailSettings(): Promise<EmailSettings> {
  ...
  return {
    ...
    contactRecipients: listValue(value, "contactRecipients"),
    leadRecipients: listValue(value, "leadRecipients"),
    senderName: textValue(value, "senderName", "Leehov Turismo"),
    replyTo: textValue(value, "replyTo"),
    footerText: textValue(value, "footerText", "Leehov Turismo"),
    whatsapp: textValue(value, "whatsapp"),
  };
}
...
export async function getServerTrackingSettings(): Promise<TrackingSettings> {
  ...
  return {
    gaMeasurementId: validTrackingId(
      textValue(value, "gaMeasurementId"),
      /^G-[A-Z0-9]{4,20}$/,
    ),
    gtmContainerId: validTrackingId(
      textValue(value, "gtmContainerId"),
      /^GTM-[A-Z0-9]{4,20}$/,
    ),
    metaPixelId: validTrackingId(
      textValue(value, "metaPixelId"),
      /^[0-9]{5,30}$/,
    ),
  };
}
