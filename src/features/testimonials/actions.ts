...
} from "@/features/testimonials/google-business";
import {
...
export async function setTestimonialActiveAction(
  ...
  const { error } = await supabase
    ...
    .update({
      active,
      featured: active ? undefined : false,
      updated_by: profile.id,
    })
    .eq("id", id);
  if (error) return { success: false, message: error.message };
  ...
  return {
    success: true,
    message: active ? "Depoimento ativado." : "Depoimento desativado.",
  };
}
...
export async function deleteTestimonialAction(
  ...
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error || !data)
    return { success: false, message: "Depoimento não encontrado." };
  if (data.active)
    return {
      success: false,
      message: "Desative o depoimento antes de excluí-lo.",
    };
  ...
  return {
    success: true,
    message:
      "Depoimento excluído. A imagem permanece disponível na biblioteca.",
  };
}

export async function setGoogleReviewVisibilityAction(
  id: string,
  visible: boolean,
  featured = false,
): Promise<TestimonialActionResult> {
  ...
  const { error } = await supabase
    ...
    .update({
      visible,
      featured: visible ? featured : false,
      updated_by: profile.id,
    })
    .eq("id", id);
  if (error) return { success: false, message: error.message };
...
export async function saveGoogleBusinessSettingsAction(
  ...
  const oauthConfigured = getGoogleOAuthConfiguration().configured;
  const placesConfigured = getGooglePlacesConfiguration().configured;
  const connection = await getLiveGoogleConnection();
  if (
    parsed.data.enabled &&
    !placesConfigured &&
    (!oauthConfigured || !connection?.location_id)
  ) {
    return {
      success: false,
      message:
        "Configure a Places API ou conecte o OAuth e confirme a localização da Leehov antes de ativar avaliações do Google.",
    };
  }
  const supabase = await createClient();
  const { error } = await supabase
  ...
  return {
    success: true,
    message: "Preferências do Google Business atualizadas.",
  };
}
...
export async function syncGoogleReviewsAction(): Promise<TestimonialActionResult> {
  ...
  try {
    ...
    for (const review of result.newReviews) {
      await emitWebhookEvent("google_review.created", {
        googleReviewId: review.id,
        rating: review.rating,
      });
      if (review.rating <= 3)
        await emitWebhookEvent("google_review.low_rating", {
          googleReviewId: review.id,
          rating: review.rating,
        });
    }
    ...
    return {
      success: true,
      message: `${result.total} avaliações sincronizadas: ${result.created} novas e ${result.updated} atualizadas.`,
    };
  } catch (error) {
    return {
      success: false,
      message:
        error instanceof Error
          ? error.message
          : "Não foi possível sincronizar as avaliações.",
    };
  }
}
...
export async function loadGoogleAccountsAction() {
  ...
  try {
    return {
      ...
      accounts: await listGoogleBusinessAccounts(),
    };
  } catch (error) {
    return {
...
export async function loadGoogleLocationsAction(accountResourceName: string) {
  ...
  try {
    return {
      success: true as const,
      message: "Localizações carregadas.",
      locations: await listGoogleBusinessLocations(accountResourceName),
    };
  } catch (error) {
    return {
      success: false as const,
...
export async function selectGoogleBusinessLocationAction(
  ...
  if (!parsed.success)
    return {
      success: false,
      message: parsed.error.issues[0]?.message ?? "Localização inválida.",
    };
  try {
    ...
    const account = accounts.find(
      (item) => item.resourceName === parsed.data.accountResourceName,
    );
    const location = locations.find(
      (item) => item.resourceName === parsed.data.locationResourceName,
    );
    if (!account || !location)
      return {
        success: false,
        message: "A conta ou localização não pertence à conexão autenticada.",
      };
    ...
    return {
      success: true,
      message: `Localização “${location.name}” confirmada.`,
    };
  } catch (error) {
    return {
      success: false,
      message:
        error instanceof Error
          ? error.message
          : "Não foi possível selecionar a localização.",
    };
  }
}
